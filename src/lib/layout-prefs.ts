...
/**
 * 浏览工作区右列的宽度**不在这里了**（人类 2026-09-23）。
 *
 * 以前它是这个文件里的常量 `BROWSE_RIGHT_WIDTH = 300`，而导入侧右列走
 * `tokens.css` 的 `--panel-w-right` —— 同一个东西两处定义，改一处忘一处。
 * 现在 workspace 右列只有一个值：`--panel-w-right`（类名 `w-panel-w-right`），
 * **所有工作流通用**；要调就改那一个数。
 *
 * 沿革（别当噪声，里面有一个真踩过的坑）：
 *   * 2026-09-19 把 browse 右列 300 → 375，但当时改的是令牌 —— 结果加宽落在了**导入**侧；
 *   * 2026-09-20 回到 300（它太占地方）；
 *   * 2026-09-23 统一到 `--panel-w-right: 255px`（300 −15%）；
 *     当晚又 +8% → **276px**（人类：高分屏上显窄）—— 见 `tokens.css` 那张尺寸沿革。
 *
 * 它**仍然是常量、不是偏好**：右列不给拖拽把手（`DESIGN.md` §8.6），没有自定义值可存。
 * 老 profile 里那个键必须继续被**忽略** —— 否则「曾经存过 300」会永远压住新默认。
 */
...
export function sanitizeLayout(
  ...
  return {
    ...
    // 右列宽度**不是偏好**：老存储里那个键直接忽略（值现在由 `--panel-w-right` 定，
    // 所有工作流共用；这里只是继续把老键丢掉）
  };
}
