 ...
 * 扩展名单独给），而界面形状是**给人看的**（秒、机型合成一个字段、格式显示名）。
 ...
 *
 * 两个约定：
 *   * **机型**优先用型号（`DC-G9` 比 `Panasonic` 有用），没有型号才退回厂商；
...
export function toExifData(file: FileExif): ExifData {
  const camera = file.cameraModel ?? file.cameraMake ?? undefined;
  return {
    camera: camera ?? undefined,
    lens: file.lens ?? undefined,
