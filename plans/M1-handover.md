...
5. 本文 §3 §4 —— 三个问题**已有定论**（见 `REPOSITORY.md` §6）+ 两件必须先问用户的事（虚拟化选型、Pencil 切文件）
...
## 3. 三个待定问题 —— **已定（用户 2026-09-16 确认），不要再问**

| # | 问题 | 定论 | 出处 |
| --- | --- | --- | --- |
| 1 | `:CDAY` / `:CWEEK` 宽度 | **2 位**（`08`、`15`、`33`） | `REPOSITORY.md` §3.2 |
| 2 | 目录透传与模版作用范围 | 模版**只求值一次**得到 target 根；source 的相对目录层级**原样搬到 target 根之下**（几层搬几层，不逐层套模版）；**只透传「含可导入图片」的子目录**（空目录 / 只有非图片的目录不建）；根下直接的文件落在 target 根 | `REPOSITORY.md` §4.2 |
| 3 | 序号回绕后又撞名 | **加数字后缀**：`_01` → `_02` → …（两位用满涨位数 `_100`），**主序号不后挪**，也不放弃报错 | `REPOSITORY.md` §3.4 |

另两条也已定：库 ID 用**已实现的 16 位可排序 base62**（`store/ids.rs`；Gid 互操作见 `FUTURE.md` G15）；
`photos/` 目录名 M1 **固定**（可配置见 `FUTURE.md` G14）。全部落点见 `REPOSITORY.md` §6「已定的决策」表。
