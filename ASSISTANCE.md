...
- **待办**：**品牌色已微调为主色 `#52C6AB` / 辅色 `#F0B033`**（`DESIGN.md` §1.1、`tokens.css` 已改），
  **Pencil 画布里的变量还是旧值** `#8DB8AA` / `#EECC55`。等你下次打开 `design/main.pen` 时，
  我可以一次性把画布变量同步过去（或者你手动改两个变量也行）。
  `DESIGN.md` §9.1 的「①↔② 手工同步」指的就是这一步。
