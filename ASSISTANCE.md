...
### A′. 跨屏 DPI 卡死：先做 1 分钟判定（挡住 A 表里的「跨屏拖拽」一项）

**现状**：您已经跑过一次 —— **从崩溃变成了卡死**（窗口「未响应」、至今开着）。
这比崩溃好办：崩溃丢现场，卡死时线程都还在、日志还读得到。

**日志证据**（`/tmp/raybend-desktop.log` 的现场）：

- **全程没有一条 `Resized` / `ScaleFactorChanged`** —— 我们的渲染代码**根本没收到通知**；
  事件只有 `Focused` 与 `Moved`，日志恰好停在最后一条 `Moved` 之后；
- 进程 `Not Responding`，CPU 累计只有 **5 秒** —— 是**等待型**卡住，不是空转；
- 没有任何「等锁」日志，且命令路径的锁纪律经查是干净的（wgpu 调用在锁外）——
  **不是我们持锁卡住**。

**判定实验（1 分钟，不用改代码、不用重启）**：把**主窗口**（不透明的那块 RayBend 主界面）
也跨屏来回拖几次。

- 主窗口**正常** → 触发条件是**透明窗口** —— 正是 `AGENTS.md` §6.1 登记的第 1 号风险命中了；
- 主窗口**也卡死** → 是 wry / WebView2 在 Windows 上的跨屏 DPI 问题本身，与透明无关。

**两种结果都有预案**：前者换「透明挖洞」的实现路线（`FUTURE.md` A3 已登记 CEF 运行时，
另有若干 Windows 合成方案可评）；后者把风险升级为项目级、重新评估渲染架构。
**判定完请把结论（哪一类）告诉我，再顺手把卡死的窗口关掉。**
