...
//!
//! ```text
//! <库根>/cache/full/1234/latest-v6.avif     ← 编辑结果
//! <库根>/cache/full/1234/raw-v6.avif        ← RAW 基础解码
//! ```
//!
//! * `issue` —— `sooc` / `raw` / `latest`（**SOOC 不进缓存**：那就是原文件本身）；
//! * `v<pipeline>` —— 渲染管线版本（`PIPELINE_VERSION`）。算法一改，旧文件自动变孤儿，
//!   与缩略图那套 `render_sig` 同一个思路；
//! * 编辑参数**不进文件名**：编辑一次覆盖同一个 `latest-*.avif`（一个 issue 一份文件），
...
impl FullCache {
    ...
    /// 某个资产某个 issue 的缓存文件路径。
    #[must_use]
    pub fn path_for(&self, asset_id: i64, issue: &str, pipeline_version: u32) -> PathBuf {
        self.root
            .join(asset_id.to_string())
            .join(format!("{issue}-v{pipeline_version}.avif"))
    }

    /// 读缓存（不存在 / 读不动都返回 `None` —— 缓存不该让界面报错）。
    #[must_use]
    pub fn read(&self, asset_id: i64, issue: &str, pipeline_version: u32) -> Option<Vec<u8>> {
        let path = self.path_for(asset_id, issue, pipeline_version);
...
mod tests {
    ...
    fn write_then_read_round_trips_and_is_atomic() {
        ...
        assert!(cache.read(7, "latest", 6).is_none(), "一开始没有");
        cache.write(7, "latest", 6, b"hello avif").expect("写");
        assert_eq!(cache.read(7, "latest", 6).as_deref(), Some(&b"hello avif"[..]));
        // 临时文件不许留下
    ...
    #[test]
    fn versions_and_issues_do_not_collide() {
        let (_dir, cache) = cache();
        cache.write(1, "latest", 6, b"v6").expect("写");
        cache.write(1, "raw", 6, b"raw").expect("写");
        cache.write(1, "latest", 7, b"v7").expect("写");
        assert_eq!(cache.read(1, "latest", 6).as_deref(), Some(&b"v6"[..]));
        assert_eq!(cache.read(1, "raw", 6).as_deref(), Some(&b"raw"[..]));
        assert_eq!(cache.read(1, "latest", 7).as_deref(), Some(&b"v7"[..]));
        // 不同资产互不影响
        assert!(cache.read(2, "latest", 6).is_none());
    }

    #[test]
    fn overwrite_replaces_the_same_file() {
        let (_dir, cache) = cache();
        cache.write(3, "latest", 6, b"first").expect("写");
        cache.write(3, "latest", 6, b"second").expect("再写");
        assert_eq!(cache.read(3, "latest", 6).as_deref(), Some(&b"second"[..]));
    }

    #[test]
    fn invalidate_removes_only_that_asset() {
        let (_dir, cache) = cache();
        cache.write(1, "latest", 6, b"a").expect("写");
        cache.write(1, "raw", 6, b"b").expect("写");
        cache.write(2, "latest", 6, b"c").expect("写");
        assert_eq!(cache.invalidate(1), 2);
        assert!(cache.read(1, "latest", 6).is_none());
        assert!(cache.read(1, "raw", 6).is_none());
        assert_eq!(cache.read(2, "latest", 6).as_deref(), Some(&b"c"[..]));
    ...
    fn clear_removes_everything_and_is_idempotent() {
        let (_dir, cache) = cache();
        cache.write(1, "latest", 6, b"a").expect("写");
        cache.write(2, "latest", 6, b"b").expect("写");
    ...
    fn empty_files_are_treated_as_missing() {
        let (_dir, cache) = cache();
        cache.write(9, "latest", 6, b"").expect("写空");
        assert!(cache.read(9, "latest", 6).is_none(), "空文件当没有（半个文件的兜底）");
    }
}
