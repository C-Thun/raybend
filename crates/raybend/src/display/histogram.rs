...
//! 所以它和缩放、色彩一样属于 Rust；前端只拿到 256 个整数去画曲线。
//!
//! # 从哪张图统计
//!
...
//! * **逐色阶一桶（256）**：人类 2026-09-19 要求曲线按像素级精度构筑 ——
//!   桶就是色阶，前端每个桶一个采样点，看不出台阶；
//! * 缩略图已经过 EXIF 方向与 3:1 夹取，统计的是**用户实际看到的那张图**。
//!
//! # 口径
//!
//! * **RGB 三通道叠加**（人类 2026-09-18 定：本期先做合成直方图，通道分离进 `FUTURE.md`）；
//! * 桶边界按 0..=255 均分（256 桶 ⇒ 一桶一个色阶，桶 i 含色阶 i），最后一个桶含 255；
