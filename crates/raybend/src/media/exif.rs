...
pub enum TakenAtSource {
    ...
    FileMtime,
    /// **从同名的姊妹文件（位图）继承**来的时间（见 `media::pairing`）。
    ///
    /// 典型情形：相机没给 RAW 写 `DateTimeOriginal`，而同一张照片的 JPG 有 ——
    /// 那张 JPG 的时间就是这张照片的时间，只是这次不是从**这个文件**里读到的。
    /// 不继承的后果不是「时间不准」那么轻：按时间分组时两种格式会落进不同的片。
    Sibling,
}
