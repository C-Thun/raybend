...
| G16 | **查看照片的 DOM 版简易实现（M1-5 评估后不做）** | `design/main.md` §3.2 的交互表里写了「双击切查看、滚轮平滑缩放、Esc 返回」。M1-5 规划时评估过做一版 DOM 实现（`transform: scale` + 拖拽），**决定不做**：M2-W3 的正式查看器是 wgpu 直绘（`AGENTS.md` §6.1 的透明挖洞），DOM 版几乎必然被替换，而导入流程并不需要逐张放大确认。**登记原因**：免得 M2 开工时又把这件事重新纠结一遍 | M2-W3 做查看器时一并实现 |
