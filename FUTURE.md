...
* **真正的收敛点**：**Rust 按规范渲染出位图 → 前端只负责拖拽/缩放 → 渲染结果按 `render_signature` 缓存**。
  这属于未来显影/编辑里程碑的原生视口；届时「缩略图 / 看图 / 导出 / 入库」共用同一条渲染管线，
  两条路并成一条，缓存键里已经预留了 `render_signature`（含 `pipeline_ver`）。
* **触发条件**：未来显影/编辑视口开工（或在此之前有人被这个观感差异咬到两次）。
...
| G16 | **查看照片：DOM 版已产品化；原生视口留给显影** | M2-W2 已完成 DOM 看图、胶片带、2–4 图对比、缩放/位移同步、评级打标与三态 chrome；M2 不再把它替换成 wgpu。原生 wgpu 透明挖洞已通过 spike，未来用于显影/编辑视口；届时保持现有状态/交互契约，只替换需要原生像素管线的渲染层。 | DOM 查看器已完成；原生视口随显影里程碑启动 |
...
* **状态**：🗓 已排期 `PLAN.md` M4-W3（用集合承接，不另造一套桶）
