...
* **导入 / 导出的格式范围（人类 2026-09-24 定；同日补充 HEIC）**：
  * **导入**：**jpg / tiff / png / webp / avif + heic，共 6 种**；
  * **导出**：**jpg / tiff / png / webp / avif，共 5 种**（**不含 HEIC**）。
  * ⚠️ 实现前提：
    * `image` crate 的 avif feature **只有编码器没有解码器** ——「导入 avif」
      需要接解码器（`avif-native` feature / dav1d，或别的解码 crate）；
      眼下用户往库里放 `.avif`，那条路走的是占位图（已登记的坑，不是新问题）；
    * `image` 完全不支持 HEIC 编解码 ——「导入 heic」要接外部解码器（libheif 系）；
      导出不涉及它，没有编码器的问题。
* **上面两条清单的定语是「位图格式」；RAW 另算（人类 2026-09-24 同日定）**：
  * **导出：不支持** —— 我们不导出 RAW（导出的是 issue 渲染结果 / 位图）；
  * **导入：尽量支持**，能力跟着 **rawler 基础库**走（候选与后备案见本文件 §B）：
    普通马赛克（Bayer）先支持全；早期的 **X3**（Sigma Foveon）可以不支持；
...
* **现状对应**：decode 已经只做一次（线性源常驻显影线程，**只在换照片时重解**）；
  ...
  缺的是：**分期边界与缓存**（现在每帧从头跑整条链）、**视口裁切计算**、**限流**。
