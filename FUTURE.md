...
## 库与磁盘的一致性：进目录「清点人头」（多删少补）

**人类 2026-09-18 提出**：浏览侧进某个目录时，要拿 catalog 里记的文件数据与磁盘上实际的文件**对一遍**——
库里多了（文件没了）就删掉那条，库里少了（磁盘有、库里没有）就补上缺失文件的数据；
...
* **真正的收敛点**：**Rust 按规范渲染出位图 → 前端只负责拖拽/缩放 → 渲染结果按 `render_signature` 缓存**。
  这正是 W3 的显影视口要做的事；届时「缩略图 / 看图 / 导出 / 入库」共用同一条渲染管线，
  两条路并成一条，缓存键里已经预留了 `render_signature`（含 `pipeline_ver`）。
* **触发条件**：W3 显影视口开工（或在此之前有人被这个观感差异咬到两次）。
...
| G16 | **查看照片：DOM 版基础实现（已落地）+ 后续演进** | 2026-09-16 之后：人类要求「先做一版基础的（不带胶片带）」，**纯前端**版已实现（`src/features/viewer`：store 与视图分离、滚轮锚点缩放、双击适配↔100%、Esc 返回、大图按需取）。**后续**：胶片带、`Ctrl` 多选对比、评级/打标、以及 M2-W3 的 wgpu 直绘版（那时**换渲染层、不换状态与交互** —— 这正是把 store 拆出来的原因）。原条目（M1-5 的评估）保留在下面： `design/main.md` §3.2 的交互表里写了「双击切查看、滚轮平滑缩放、Esc 返回」。M1-5 规划时评估过做一版 DOM 实现（`transform: scale` + 拖拽），**决定不做**：M2-W3 的正式查看器是 wgpu 直绘（`AGENTS.md` §6.1 的透明挖洞），DOM 版几乎必然被替换，而导入流程并不需要逐张放大确认。**登记原因**：免得 M2 开工时又把这件事重新纠结一遍 | M2-W3 做查看器时一并实现 |
...
## issue 与 SOOC（人类 2026-09-19 记一笔，**本次不做**）
