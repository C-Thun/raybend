...
* **单拎出来说**：`BaselineExposureOffset` **本身就有价值** —— 它就是「RAW 本来偏暗」的那个补偿量（各家能差 ±1 EV）。
  **但它不必靠 DCP 拿到**：同一个量能从内嵌 JPEG 反推，更省事且无许可问题（见 `PLAN.md` M3-W6b 的 RAW/直出拟合方案）。
...
| G20 | **Gallery / 外部编辑器** | 外部编辑产物入库统一管理（新 flow + 与 browse 关联跳转）；**取回链路跑不顺**，现在只做「纯单向输出」。详见文末「Gallery / 外部编辑器」 | 单向输出已排期 **M4-W6**；Gallery 本体未排期 |
...
* **XMP 互操作**：sidecar 写入、Lightroom `crs:`/`lr:` 基础项交换、「DB 为真相源、XMP 为通道」冲突规则；
  issue 的 XMP 表示契约已在 `docs/issue-xmp-contract.md` 定案；sidecar 读写仍属后续互操作实现
...
## Gallery / 外部编辑器（人类 2026-09-24 记一笔；**单向输出已排期 M4-W6**）
...
- **已排期 M4-W6**（`PLAN.md`，2026-09-24 定案）；M4-W4 导出格式清单已含 TIFF（16-bit 无损，外部编辑前置）。
...
它们收到的是我们的渲染 TIFF，其 RAW 专属模块不可用，属预期（`PLAN.md` M4-W6）。
