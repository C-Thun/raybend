...
13. **复用优先于重复硬编码**（人类 2026-09-19 明确、且从项目开头反复强调）：
...
14. **本地应用实时性优先，丢掉 web 式缓存思维**（人类 2026-09-19 明确）：
...
15. **一轮清单要连续做完，不要做一两项就回来确认**（人类 2026-09-19 明确）：
    - 人类一次给一批事项（todos）时，默认是「**一直做到没有可做的**」，
      不要每做完一两项就停下等人点头。
    - 停下来只有两种正当理由：① **真的需要人类协助**（写进 `ASSISTANCE.md` 对应分节，见 #12）；
...
- **一次规划只覆盖一个工作单元** 定义`工作单元`最小颗粒度为一个波次，禁止将一个波次内切分的小颗粒度任务作为一次工作范围，工作按波次进行，波次中**禁止停下**。
