...
>
> **本目录不受 `ASSISTANCE.md` 约束**（用户 2026-09-17 定）：官网开发**不被应用本体那份清单阻塞**，
> 这里需要人做的事（图片素材 / 截图 / 域名与 Pages 配置等）**直接在对话里提**，不写进 `ASSISTANCE.md`。
...
- **保真度实测**（把矢量光栅化回源尺寸后逐像素比）：汉字 IoU 98.8%、拉丁大字 95.7%、
  手写口号 94.9%/90.8%；**遗漏 ≤ 0.6%**（细尖几乎不丢），**多出 0.9%～9.6%**
  （细笔画被拉粗一点，这是「模糊 + 二值化」的固有偏差，在高清字面上看不出，
  但在 3 倍放大对比图里能看出来）。想要更高保真可以：拆小阈值 / 换 `vtracer`（保留细笔画更好、输出更大），
  或者干脆不用矢量（见下）。
